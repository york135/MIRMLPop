...
if __name__ == "__main__":
    ...
    for i in range(len(data)):
        # commonvoice
        if data[i].get('aligned_lyrics') is None:
            assert len(data[i]['lyric']) == len(data[i]['pronounce'])
            for j in range(len(data[i]['lyric'])):
                if data[i]['lyric'][j] not in character_list:
                    character_list.append(data[i]['lyric'][j])

                if '(' in data[i]['pronounce'][j] and '((' not in data[i]['pronounce'][j]:
                    cur_pronoun = data[i]['pronounce'][j].split('(')[1].split(')')[0]
                else:
                    cur_pronoun = data[i]['pronounce'][j].split('(')[0]
